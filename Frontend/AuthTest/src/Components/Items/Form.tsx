...
export default function Form() {
  ...
  // Παίρνουμε από το Zustand store:
  // - token: αν υπάρχει, redirect αυτόματα (είναι logged in)
  // - isLoading: αν τρέχει login ή register (ορίζουμε state loading)
  ...
  // Αν ο χρήστης έχει ήδη token, μεταφερόμαστε στη LandingPage
  useEffect(() => {
    if (token) {
  ...
  // - κάνουμε reset τα πεδία
  // - καθαρίζουμε τυχόν React Hook Form errors
  // - καλούμε clearError() για να σβηστεί το σφάλμα από το store
  useEffect(() => {
    reset()
    clearErrors()
    clearError()
  ...
  const onSubmit: SubmitHandler<FormValues> = async data => {
    if (isLogin) {
      // Κάνουμε login μέσω του Zustand store:
      await login(data.email!, data.password1)
      // Αν το login πετύχει, το effect που κοιτάει το token θα κάνει redirect.
      // Αν αποτύχει, το error θα εμφανιστεί παρακάτω.
    } else {
      // Registration: εδώ απευθείας axios call (μπορείτε να φτιάξετε και thunk στο store αν θέλετε)
      try {
